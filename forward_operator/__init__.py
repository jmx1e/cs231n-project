...
@register_operator(name='hdr_gamma')
class HDRGammaOperator(Operator):
    """HDR operator with gamma correction"""
    
    def __init__(self, device='cuda', scale=2, sigma=0.05, gamma=2.2):
        super().__init__(sigma)
        self.device = device
        self.scale = scale
        self.gamma = gamma
    ...
    def __call__(self, x):
        # Apply gamma correction then HDR mapping
        # Convert from [-1, 1] to [0, 1] for gamma correction
        x_normalized = (x + 1) / 2
        gamma_corrected = torch.pow(torch.clamp(x_normalized, 0, 1), 1.0/self.gamma)
        hdr_mapped = gamma_corrected * self.scale
        # Convert back to [-1, 1] range
        return torch.clamp(hdr_mapped * 2 - 1, -1, 1)


@register_operator(name='hdr_adaptive_gamma')
class HDRAdaptiveGamma(Operator):
    """HDR operator with pixel-wise adaptive gamma encoding"""
    
    def __init__(self, device='cuda', scale=2, sigma=0.05, alpha=1.5, beta=0.5):
        super().__init__(sigma)
        self.device = device
        self.scale = scale
        self.alpha = alpha  # Controls dynamic range allocation
        self.beta = beta    # Sets baseline nonlinearity
    ...
    def __call__(self, x):
        ...
        q_max = torch.max(x_clamped)
        
        # Compute pixel-wise gamma values
        # gamma_{i,j} = alpha * (q_{i,j} / q_max) + beta
        gamma_map = self.alpha * (x_clamped / (q_max + 1e-8)) + self.beta
        
        # Apply pixel-wise adaptive gamma correction
        gamma_corrected = torch.pow(x_clamped, gamma_map)
        
        # Apply scale factor
        hdr_mapped = gamma_corrected * self.scale
        
        # Convert back to [-1, 1] range and clip
        return torch.clamp(hdr_mapped * 2 - 1, -1, 1)


@register_operator(name='hdr_highlight_clip')
class HDRHighlightClip(Operator):
    def __init__(self, device='cuda', scale=2, sigma=0.05):
        super().__init__(sigma)
        self.device = device
        self.scale = scale
    ...
    def __call__(self, data):
        # [-1, 1] -> [0, 1]
        data_pos = (data + 1.0) / 2.0
        # Scale by factor
        scaled = data_pos * self.scale
        # Clip to [0, 1]
        clipped = torch.clip(scaled, 0, 1)
        # [0, 1] -> [-1, 1]
        return clipped * 2.0 - 1.0


@register_operator(name='hdr_shadow_clip')
class HDRShadowClip(Operator):
    def __init__(self, device='cuda', scale=2, sigma=0.05):
        super().__init__(sigma)
        self.device = device
        self.scale = scale
    ...
    def __call__(self, data):
        # [-1, 1] -> [-1, 0]
        data_neg = (data - 1.0) / 2.0
        # Scale by factor
        scaled = data_neg * self.scale
        # Clip to [-1, 0]
        clipped = torch.clip(scaled, -1, 0)
        # [-1, 0] -> [-1, 1]
        return clipped * 2.0 + 1.0


@register_operator(name='hdr_baseline')
class HDRBaseline(Operator):
    """Baseline naive solution for HighDynamicRange operator"""
    ...
    def __call__(self, data):
        # Generalized naive solution: divide all pixels by scale factor
        return data / self.scale


@register_operator(name='hdr_highlight_clip_baseline')
class HDRHighlightClipBaseline(Operator):
    """Baseline naive solution for HDRHighlightClip operator"""
    ...
    def __call__(self, data):
        # Generalized naive solution: x = (1/scale) * (x_out - (scale-1))
        # For scale=2: x = 0.5 * (x_out - 1)
        # Perfectly recovers x ∈ [-1, (2-scale)/scale] except noise
        return (data - (self.scale - 1)) / self.scale


@register_operator(name='hdr_shadow_clip_baseline')
class HDRShadowClipBaseline(Operator):
    """Baseline naive solution for HDRShadowClip operator"""
    ...
    def __call__(self, data):
        # Generalized naive solution: x = (1/scale) * (x_out + (scale-1))
        # For scale=2: x = 0.5 * (x_out + 1)  
        # Perfectly recovers x ∈ [(scale-2)/scale, 1] except noise
        return (data + (self.scale - 1)) / self.scale
